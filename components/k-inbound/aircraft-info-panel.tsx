...
function Row({ label, value, cls = "text-white" }: { label: string; value: string; cls?: string }) {
  return (
    <div className="grid grid-cols-[110px_1fr] gap-1 py-[3px] group cursor-default">
      ...
      <span className={`text-[13px] font-semibold transition-all duration-150 group-hover:brightness-125 ${cls} break-words`}>{value}</span>
    </div>
  )
}
...
export function AircraftInfoPanel({ flight }: Props) {
  ...
  return (
    <div className="shrink-0 flex flex-col bg-black/75 backdrop-blur-sm border border-[#4a9eff]/30 rounded-xl p-4 font-mono text-white">
      <div className="text-[11px] uppercase tracking-wider text-[#cbd5e1] mb-2 hover:text-white transition-colors duration-150 cursor-default">Aircraft Info</div>
