...
// Hallyu News RSS 수집 — Koreaboo / Allkpop / Soompi 매일 01:00 UTC
// 7일 이상 된 뉴스 자동 삭제
...
const FEEDS = [
  ...
  { source: "allkpop",  url: "https://www.allkpop.com/feed" },
...
// RSS item 에서 썸네일 URL 추출 — media:content / media:thumbnail / enclosure 순서로 시도
...
export async function GET(request: Request) {
  ...
  for (const feed of FEEDS) {
    try {
      ...
      console.log(`[collect-hallyu-news] ${feed.source}: ${items.length}개 항목`)
      ...
      if (rows.length === 0) {
        summary[feed.source] = { inserted: 0 }
        continue
      }
      ...
      if (error) {
        console.error(`[collect-hallyu-news] ${feed.source} upsert 실패:`, error.message)
        summary[feed.source] = { inserted: 0, error: error.message }
      } else {
        const inserted = data?.length ?? 0
        summary[feed.source] = { inserted }
        totalInserted += inserted
        console.log(`[collect-hallyu-news] ${feed.source}: ${inserted}건 저장`)
      }
    } catch (err) {
      console.error(`[collect-hallyu-news] ${feed.source} 수집 실패:`, err)
      summary[feed.source] = { inserted: 0, error: String(err) }
    }
  }
