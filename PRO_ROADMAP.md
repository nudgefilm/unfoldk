...
- [x] 시청 기록 기반 Claude 개인화 추천 프롬프트 구축
- [x] Free: 장르/키워드 기반 일반 추천 (현재 동작)
- [x] Pro: 시청 목록 + 평점 분석 → 취향 프로파일 → 맞춤 추천
- [x] 추천 결과에 "왜 이 드라마를 추천하는지" 개인화 이유 노출
- **가치 제안**: 데이터가 쌓일수록 추천이 정확해지는 락인 효과
- **관련 파일**: `lib/claude/recommend-dramas.ts`, `app/api/dramas/recommend/route.ts`

## ⑤ 주간 한류 리포트 (예상 4~6시간)

- [x] 매주 월요일 자동 생성: "This Week in Hallyu" 원페이퍼 리포트
- [x] 7섹션 구성:
...
- [x] Claude Haiku 자동 생성 → weekly_reports 테이블 캐싱
- [x] Pro 전용 이메일 발송 + /mypage 내 리포트 아카이브 열람
- [x] 각 섹션에 해당 서비스 "자세히 보기 →" CTA 포함
...
| ④ K-Drama 맞춤 추천 | ✅ 완료 | 2026-05-23 |
| ⑤ 주간 한류 리포트 | ✅ 완료 | 2026-05-23 |
